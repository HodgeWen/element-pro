...
表单是web应用的核心之一. element-pro的表单在原来element-plus的基础上做了大量的优化.
一是现在可以用更少的代码来完成所需的功能, 所有的表单功能将会被自动套上一层el-form-item组件, 并接管数据.
二是表单默认使用el-grid来完成响应式布局.
三是form-item组件更加精简, 将此前大量的函数和方法放到了form中以便减少内存的消耗.
四是不再依赖async-validator库去实现表单的校验功能
